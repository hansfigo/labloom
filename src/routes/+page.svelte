<script lang="ts">
	...
	const transformed = transformArray(asisten, 3);
</script>
...
<nav class="flex w-full justify-center bg-secondary-600 py-8">
...
<div class="flex flex-col gap-2 relative overflow-y-scroll justify-center items-center w-full">
	<div class="container md:px-0 px-20">
		<div class="w-full mt-12 px-12">
			<div class="w-full bg-secondary-600 relative px-12 rounded-[2rem] shadow-md py-28">
				<h1 class="text-white text-center pb-6">The Next-Gen Assistant Platform</h1>
				<p class="text-center text-tertiary-300 font-normal text-2xl">
			...
			<section class="grid grid-cols-2 content-start md:grid-cols-4 gap-2 md:gap-4">
				{#each transformed as e}
					<div class="grid gap-2 content-start	">
						{#each e as x}
							<div
								class="  h-fit cursor-pointer w-fit rounded-xl scale-100 xl:rounded-3xl shadow-xl overflow-hidden"
							>
								<section
									class="absolute z-[1] asisten-card h-full w-full bg-gradient-to-t from-slate-800 opacity-70"
								/>
								<img
									class="h-auto asisten-img object-fill max-w-full duration-700 transition-all ease-in-out"
									src={x.gambar}
									alt={`photo-${x.nama}`}
								/>
								<p
...
<style lang="postcss">
	h1 {
		@apply text-[3rem] md:text-[3.5rem] font-semibold;
	}
